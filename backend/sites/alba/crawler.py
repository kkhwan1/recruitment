...
class AlbaCrawler:
    ...
    def __init__(self, headless: bool = True):
        ...
        """
        self.logger = setup_logger("AlbaCrawler")
    ...
    def _close_popups(self):
        ...
        try:
            ...
            for selector in popup_selectors:
                try:
                    popup_btn = self.page.query_selector(selector)
                    if popup_btn and popup_btn.is_visible():
                        popup_btn.click()
                        time.sleep(0.5)
                        self.logger.debug(f"팝업 닫음: {selector}")
                except:
                    continue
        except Exception as e:
            self.logger.debug(f"팝업 닫기 시도 중 오류 (무시됨): {e}")

    def search(self, keyword: str) -> bool:
        """
        키워드로 검색 (알바천국의 검색 메커니즘에 맞게 구현)
        ...
        try:
            ...
            self.logger.debug(f"검색 후 URL: {current_url}")
    ...
    def get_job_list(self) -> List[str]:
        ...
        job_links = []
        try:
            # JavaScript로 링크 수집 - 알바천국의 실제 패턴: /job/Detail?adid=
            links_data = self.page.evaluate("""
                () => {
                    const links = [];

                    // 알바천국의 실제 공고 링크 패턴: /job/Detail?adid=숫자
                    document.querySelectorAll('a[href]').forEach(el => {
                        const href = el.getAttribute('href');
                        if (href && href.includes('/job/Detail') && href.includes('adid=')) {
                            links.push(href);
                        }
                    });
                    ...
                    return [...new Set(links)].map(link => {
                        if (link.startsWith('http')) {
                            ...
                            return 'https://www.alba.co.kr' + link;
                        } else {
                            return 'https://www.alba.co.kr/' + link;
                        }
                    });
            ...
            if job_links:
                self.logger.debug(f"공고 링크 샘플: {job_links[0]}")
        ...
        for attempt in range(max_retries):
            try:
                ...
                () => {
                    ...
                    // 제목 추출 (h1, h2, .title 등)
                    const titleSelectors = ['h1', 'h2', '.title', '[class*="title"]', '.job-title'];
                    ...
                    // 급여 패턴 찾기
                    const salaryMatch = detail.match(/(?:급여|시급|일급|월급|연봉)[\\s\\n]*:?[\\s\\n]*([^\\n]{0,100})/i);
                    if (salaryMatch) {
                    ...
                    if (locationMatch) {
                        ...
                        // 지역 패턴으로 찾기
                        const regionMatch = detail.match(/(서울|경기|인천|부산|대구|광주|대전|울산|세종|강원|충북|충남|전북|전남|경북|경남|제주)[^\\n]{0,100}/);
                        if (regionMatch) {
                    ...
                    // 지원자격/모집조건 패턴 찾기
                    const conditionsMatch = detail.match(/(?:지원자격|자격요건|모집조건|지원조건)[\\s\\S]{0,800}/i);
                    if (conditionsMatch) {
                        result.conditions = conditionsMatch[0].trim();
                    }

                    // 모집요강 패턴 찾기
                    const recruitMatch = detail.match(/(?:모집요강|채용내용|상세내용|업무내용)[\\s\\S]{0,1500}/i);
                    if (recruitMatch) {
                        result.recruit_summary = recruitMatch[0].trim();
                    }

                    // 등록일/마감일 패턴 찾기
                    const dateMatch = detail.match(/(?:마감일|등록일|접수기간|게시일)[\\s\\n]*:?[\\s\\n]*(\\d{4}[\\s\\-./]\\d{1,2}[\\s\\-./]\\d{1,2}[^\\n]{0,30})/);
                    if (dateMatch) {
    ...
    def _extract_fields_from_detail(self, job_info: Dict) -> Dict:
        ...
        if not job_info.get("recruit_summary"):
            recruit_match = re.search(r'(?:모집요강|채용내용|상세내용|업무내용)[\s\S]{0,1500}(?=접수기간|기업정보|$)', detail, re.IGNORECASE)
            if recruit_match:
    ...
    def save_results(self, keyword: str, jobs: List[Dict]):
        ...
        filename = f"alba_{safe_keyword}_{timestamp}.json"
